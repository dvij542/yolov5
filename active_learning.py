...
def get_map_of_labels(opt, model_path, path_filter, device, hyp):
    ...
    imgsz, imgsz_test = [check_img_size(x, gs) for x in opt.img_size]  # verify imgsz are gs-multiples
    task = 'test'
    dataloader = create_dataloader(path_filter, imgsz, 1, gs, opt, pad=0.5, rect=True, prefix=colorstr(f'{task}: '))[0]
    loss = 0
...
def transfer_images(active_learning_path, train_path, img_paths) :
    ...
    for img_path in img_paths :
        ...
        os.system("mv " + label_path + " " + label_path.replace("active_learning", "train"))
...
def active_learning(opt):
    ...
    while not done :
        ...
        print(total_effort)
        total_effort += no_each_time*(1 - get_map_of_labels(opt, "trial.pt", curr_paths, device, hyp))
        print(total_effort)
        transfer_images(active_learning_path, train_path, curr_paths)
